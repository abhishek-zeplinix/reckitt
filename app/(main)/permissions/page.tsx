...
const PermissionsPage = () => {
  ...
  const checkboxBodyTemplate = (rowData: any, { rowIndex, field }: { rowIndex: any; field: any }) => (
    <Checkbox
      checked={rowData[field]}
      onChange={() => toggleCheckbox(rowIndex, field)}
    />
  )
                    ...
                    <div className="card mt-4">
                        <DataTable value={permissions} showGridlines>
                            ...
                            <Column
                                field="read"
                                header="Read"
                                body={(rowData, options) => checkboxBodyTemplate(rowData, { ...options, field: 'read' })}
                            />
                            <Column
                                field="write"
                                header="Write"
                                body={(rowData, options) => checkboxBodyTemplate(rowData, { ...options, field: 'write' })}
                            />
                            <Column
                                field="execute"
                                header="Execute"
                                body={(rowData, options) => checkboxBodyTemplate(rowData, { ...options, field: 'execute' })}
                            />
                            <Column
                                field="view"
                                header="View"
                                body={(rowData, options) => checkboxBodyTemplate(rowData, { ...options, field: 'view' })}
                            />
                            <Column
                                field="admin"
                                header="Admin"
                                body={(rowData, options) => checkboxBodyTemplate(rowData, { ...options, field: 'admin' })}
                            />
                        </DataTable>
                    </div>
