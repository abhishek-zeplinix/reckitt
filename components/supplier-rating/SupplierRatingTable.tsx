...
const SupplierEvaluationTable = ({ rules, category, evaluationPeriod, categoryName, departmentId, department, isEvaluatedData, totalScoreEvaluated, onSuccess }: any) => {
    ...
    const handleSubmit = async () => {
        const hasInvalidCapa = capaData.some((item) => !item.selectedStatus || item.selectedStatus.trim() === '');
    ...
    return (
        ...
        <div className=" w-full shadow-sm mt-3 overflow-x-auto">
            <div className="min-w-[800px]">
                <div className="flex justify-content-start">{isEvaluatedData ? <Badge value="Evaluated" severity="success"></Badge> : ''}</div>
                                    ...
                                    return (
                                        <tr key={`criteria-${key}`} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2">
                                                <InputText type="text" value={currentPercentage === 'NA' ? 'NA' : displayPercentages[key] + '%'} size={1} readOnly className="m-auto text-center" />
                                            </td>
            ...
            <div className="flex flex-col justify-content-end gap-3 mt-2 mr-2">
                ...
                <div>
                    ...
                    {isEvaluatedData ? (
                        <InputTextarea rows={5} cols={30} value={rules?.comments} disabled={isEvaluatedData} />
                    ) : (
                        <InputTextarea rows={5} cols={30} onChange={(e) => setComments(e.target.value)} value={comments} disabled={isEvaluatedData} />
                    )}
                </div>
            </div>
            ...
            {isEvaluatedData ? (
                <div className=" right-0 bottom-0 flex justify-center gap-3 mt-4">{totalScoreEvaluated <= 50 && <CapaRequiredTable onDataChange={handleCapaDataChange} depId={departmentId} existingSelections={rules?.capa} isEvaluatedData />}</div>
            ) : (
                <div className=" right-0 bottom-0 flex justify-center gap-3 mt-4">{totalScore <= 50 && isCapaRulesVisibleOnInitialRender && <CapaRequiredTable onDataChange={handleCapaDataChange} depId={departmentId} isEvaluatedData={false} />}</div>
            )}

            <div className="flex justify-content-end gap-3 mt-1 p-3">
                {!isEvaluatedData && (
                    <>
                        <Button label="Save" className="bg-primary-main hover:text-white" onClick={handleSubmit} disabled={isAnyEvaluationEmpty} />
                    </>
                )}
            </div>
        </div>
